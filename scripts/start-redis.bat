...
set NAME=medical-redis
set IMAGE=redis:7-alpine
set PORT=6379
...
set LOGFILE=%LOG_DIR%\start-redis-%TS%.log
...
if %errorlevel%==0 (
  ...
  if not %errorlevel%==0 (
    ...
    docker logs --tail 50 "%NAME%" >> "%LOGFILE%" 2>&1
    call :maybe_pause %*
    exit /b 1
  )
  goto :verify
...
docker run -d --name "%NAME%" -p %PORT%:6379 "%IMAGE%" >> "%LOGFILE%" 2>&1
if not %errorlevel%==0 (
...
call :log "[OK] Redis is running on localhost:%PORT%"
call :maybe_pause %*
exit /b 0
